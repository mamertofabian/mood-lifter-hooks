...
from lib.constants import CacheDuration, Timeouts
...
class JokeQuoteClient:
    ...
    def __init__(self):
        ...
        self.api_client = APIClient(
            cache_ttl_minutes=CacheDuration.EXTERNAL_CONTENT, timeout=Timeouts.EXTERNAL_APIS
        )
    ...
    def get_inspirational_quote(self) -> Optional[Dict[str, str]]:
        ...
        if data and isinstance(data, list) and len(data) > 0:
            ...
            if isinstance(quote_data, dict):
                return {"text": quote_data.get("q", ""), "author": quote_data.get("a", "Unknown")}
    ...
    for _ in range(3):
        ...
        if content:
            if content.get("type") == "joke":
                ...
                print(
                    f"   • Quote: \"{content['content'][:50]}...\" - {content.get('author', 'Unknown')}"
                )
