...
class JWDailyTextClient:
    ...
    def fetch_daily_text(self, date: Optional[datetime] = None) -> Optional[Dict]:
        ...
        for item in items:
            ...
            if item.get("englishSymbol", "").startswith(
                "es"
            ) and "Examining the Scriptures Daily" in item.get("publicationTitle", ""):
                ...
                if content:
                    ...
                    scripture_pattern = (
                        r'<p[^>]*class="themeScrp"[^>]*>.*?<em>(.*?)</em>.*?<em>(.*?)</em>.*?</p>'
                    )
...
def create_developer_encouragement(
    ...
    if use_lm_studio:
        ...
        full_text = (
            f"{scripture_display} {commentary}"
            if scripture_display and commentary
            else (commentary or scripture_display or "N/A")
        )
...
def generate_jw_message(
    event_type: str = "SessionStart", time_period: Optional[str] = None, use_lm_studio: bool = True
) -> Optional[str]:
    """
    ...
    return create_developer_encouragement(daily_text, use_lm_studio)
